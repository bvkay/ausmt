...
def render_js(rings) -> str:
    ...
    body = ['// Australia coastline and state boundaries for the footprint mini-map: a classic script',
            "// assigning one global, so drawer.js draws it with no fetch under a script-src 'self' CSP.",
            '// GENERATED by engine/extract/_au_outline_build.py: edit the build script, not this file.',
            '// See docs: portal internals, au-outline.js.',
            '',
            '// FORMAT: coordinates are [longitude, latitude] in WGS84, `coast` an array of closed rings',
            '// (largest first) and `borders` an array of open polylines. Same geometry as',
            '// engine/extract/_au_outline.py, held coordinate for coordinate by a test.',
            '',
            '// SOURCE AND LICENCE. The geometry is derived from Natural Earth under its own licence',
            '// terms, which ask for no attribution, and no rights are claimed over this file: it is a',
            '// locator backdrop for context, never a survey-grade or legal boundary.',
            f'//   {prov[0]}',
            f'//   {prov[1]}',
            f'//   {prov[2]}',
            '//   Inter-state boundaries are the legislated meridian and parallel segments, which are',
            '//   plain geographic facts.',
            'window.AU_OUTLINE = {',
            '  coast: [']
