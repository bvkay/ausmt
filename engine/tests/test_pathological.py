...
def test_inf_error_values_do_not_drop_the_station(tmp_path):
    ...
    dead/infinite-variance points) crashes TF assembly and drops the whole station. Pre-fix: sig()'s
    log10(inf) -> int(inf) raised OverflowError, build_portal caught it as a
