...
class Gateway:
    ...
    def __init__(self, cfg: Config, scanner=None, git_runner=None, edit_runner=None, mailer=None):
        ...
        # Publish seam (v2), same injected-callable pattern as the scanner: git_runner
        ...
        # needs the Docker socket the invariant forbids it).
        ...
        # can tell a genuinely-stuck PUBLISHING row (gateway restarted mid-publish) from
    ...
    async def _handle_submit_reserved(self, request: Request, auth: "_SubmitAuth"):
        ...
        # Zip central-directory safety BEFORE promoting the part-file. A rejection here
        # writes nothing under quarantine/ and no DB row (rejected at upload, nothing in
        # quarantine).
        try:
        ...
        # Duplicate CONTENT, not merely duplicate work-in-progress: bytes the archive
        ...
        # clamd scan of the raw zip. Fail closed: on ScanError the row STAYS RECEIVED
    ...
    def handle_status(self, token: str) -> Response:
        ...
        # to the same 404 with a byte-identical body.
    ...
    async def _apply_done(self, sub: db.Submission, done: jobs.DoneFile) -> None:
        if done.outcome == jobs.OUTCOME_VALIDATED:
            # Second clamd sweep of the unpacked tree - the runner had no network.
    ...
    async def _post_unpack_sweep(self, submission_id: str) -> str | None:
        """clamd-sweep every file under quarantine/<id>/package (bounded by the member cap).
    ...
    def _requeue_dead(self) -> None:
        ...
        'job died twice'. Tracked by a marker suffix so the second pass can tell a
    ...
    def _curator_keys(self) -> dict[str, str]:
        ...
        route works. Cheap enough to parse per-request; not cached so a config change on
    ...
    def _session_curator(self, request: Request) -> str | None:
        ...
        if expired (absolute expiry) so a stale session cannot be replayed."""
    ...
    def handle_curator_login(self, request: Request, curator_key: str | None) -> Response:
        try:
            ...
            return self._curator_503()
        # ONE atomic decision (thread-safe): the blocked-check, the key match, and the
    ...
    def _nav_context(self, request: Request, *, active: str, crumb: str) -> "curatorpage.NavContext":
        """Build the nav-shell chrome inputs for a curator page. Reads the published
    ...
    def handle_serve_state(self, request: Request) -> Response:
        """GET /gateway/curator/serve - the first-class serve-state screen.
    ...
    def handle_serve_build_detail(self, request: Request, build_ref: str) -> Response:
        """GET /gateway/curator/serve/build/{build_ref} - read-only build forensics. Matches
        ...
        and yields a 'no such build' page, never a traversal. Renders identity + the cache
    ...
    def handle_serve_update(self, request: Request, csrf: str | None) -> Response:
        """POST /gateway/curator/serve/update - write update.request (the bounded exception; the
    ...
    def handle_context_bar_js(self, request: Request) -> Response:
        ...
        same-origin EXTERNAL script (Stage 1). Same CSP reason as serve-state.js/ui.js: the
    ...
    def handle_survey_hub_js(self, request: Request) -> Response:
        """GET /gateway/curator/survey-hub.js - the survey hub's browser-side script (Stage 1):
    ...
    def handle_doi_harvest_js(self, request: Request) -> Response:
        ...
        the publications "Look up DOI" button uses (the contributor-credit model). It is the SAME code the
    ...
    def handle_collections_js(self, request: Request) -> Response:
        """GET /gateway/curator/collections.js - the Stage-3b collections editor's ONLY browser
        script: the candidate-picker filter. Same CSP reason as the other external scripts
    ...
    def handle_security_rotate(self, request: Request, code: str | None,
        ...
        ACTIVE enrolment (the collapse guard - a session alone must never rotate the secret). On a
    ...
    def handle_survey_hub(self, request: Request, slug: str, tab: str = "overview", *,
        ...
        """The per-survey hub (Stage 1; header styling). Overview & QA (default) /
    ...
    def handle_collection_create(self, request: Request, *, error: str = "",
                                 status_code: int = 200) -> Response:
        """GET /gateway/curator/collections/new: the create form - details + the candidate
    ...
    async def handle_collection_preview(self, request: Request, cid: str, form, *,
        ...
        version), validate each, and render the combined batch-diff confirm (preview
    ...
    async def handle_collection_publish(self, request: Request, cid: str, form: dict, *,
        ...
        UNDER THE LOCK from the carried spec (do NOT trust the preview - the TOCTOU lesson), verify
    ...
    async def _commit_collection(self, cid: str, operations: list, expected_shas: dict,
        ...
        # Re-enforce the guardrails on the untrusted client-carried spec BEFORE any commit
    ...
    def _edit_error_surface(self, request: Request, slug: str, form: dict, *, error: str = "",
        ...
        loses their place: harmless when a save is one section, actively bad when a save spans ten).
        Without the marker (the standalone full form) the legacy re-render is
    ...
    def handle_pid_check(self, request: Request) -> Response:
        """The curator DOI resolution chip. Session-gated GET that HEADs doi.org
    ...
    async def handle_edit_preview(self, request: Request, slug: str, form: dict) -> Response:
        ...
        # and enforces semver-greater (all version logic stays runner-side). The gateway
    ...
    async def handle_edit_confirm(self, request: Request, slug: str, form: dict) -> Response:
        """Confirm + commit: re-run the merge server-side to reproduce the EXACT bytes,
        re-hash and 409 on any mismatch with the previewed sha, then commit+push through the
        ...
        if result.get("has_fail"):
            # A validator FAIL at confirm time is the server-side guarantee, independent of the UI.
    ...
    def handle_survey_retire_confirm(self, request: Request, slug: str) -> Response:
        """GET the retirement confirmation page: session-gated. Resolves the package (404 if
    ...
    async def handle_survey_retire(self, request: Request, slug: str, form: dict) -> Response:
        """POST the retirement: the server GATES IN ORDER - session, CSRF, the last-survey
    ...
    def _build_patch(self, form: dict):
        """Turn the edit form fields into the merge patch. Top-level SCALAR fields (f_*)
        ...
        json is stdlib, not survey content - the no-yaml rule is unbroken. Returns
    ...
    def handle_curator_preview(self, request: Request, submission_id: str, subpath: str) -> Response:
        ...
        # Path containment: resolve the requested sub-path and confirm it stays UNDER the
        ...
        # Strict CSP: default-src 'self' confines what the preview can load to same-origin;
    ...
    async def handle_curator_action(self, request: Request, submission_id: str, action: str,
        ...
        if not curator_auth.csrf_ok(raw, csrf):
            ...
            # guarantee that a cross-site form cannot drive a state change.
            return self._forbidden("bad csrf token")
        if not db.is_valid_id(submission_id):
            return self._not_found()
        note = (note or "").strip()
    ...
    async def _begin_publish(self, sub: db.Submission, curator: str, note: str, action: str,
        ...
        # then run the publish in a background task so the request returns immediately.
        try:
    ...
    async def _run_publish(self, submission_id: str, slug: str | None, curator: str, note: str,
        ...
        """The v2 publish (commit-and-push ONLY, no build): single-flight under PUBLISH_LOCK,
    ...
    def _publish_blocking(self, submission_id: str, slug: str | None, curator: str, note: str,
        ...
        # Pre-flight (step 1): abort unless the checkout is clean and on main, capturing the
    ...
    def _reconcile_publishing(self) -> None:
        """Poll-loop reconciliation: a PUBLISHING row with no live in-process task means
    ...
    def _set_session_cookie(self, resp: Response, token: str) -> None:
        # Secure; HttpOnly; SameSite=Strict. Path scoped to the curator area. HttpOnly
...
# The ONLY content types the preview route serves. An allow-list, not a guess: the
...
def create_app(cfg: Config | None = None, scanner=None, git_runner=None, edit_runner=None,
    ...
    # ---- curator routes. All session-gated except login; every state-changing POST is
    ...
    # task - matching the status route's rationale.
    ...
    # ---- the first-class serve-state screen + the read-only build-detail view. Both GETs
    ...
    # The nav-shell context bar's drift chip served-build half - EXTERNAL for the same CSP reason;
    ...
    # The survey-hub script (Overview & QA render + Metadata one-section-at-a-time TOC) - EXTERNAL
    ...
    # The Stage-2a Stations tab script - EXTERNAL for the same CSP reason; the hub's Stations tab
    ...
    # The Surveys-list enrichment script - EXTERNAL for the same CSP reason; the surveys
    ...
    # ---- survey retirement. GET renders the danger-zone confirmation page (blocking read-job ->
