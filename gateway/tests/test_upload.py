...
def test_wrong_key_unauthorized(tmp_path):
    # Proven failing: with the hmac check stubbed to True, this returned 201 not 401.
    async def _body():
        async with app_client(tmp_path, scanner=scanner_clean()) as (client, _app, _gw, cfg):
...
def test_ratio_bomb_rejected_at_upload(tmp_path):
    ...
    # Proven failing: with the ratio guard disabled the upload returned 201, state SCANNED.
    async def _body():
...
def test_midstream_cap_when_content_length_passes_gate(tmp_path):
    ...
    # Proven failing: with the mid-stream cap removed, the 1.5-MiB part was written whole
...
def test_duplicate_sha_conflicts_after_the_first_was_approved_and_published(tmp_path):
    ...
    FAILS IF the second submit of identical bytes is accepted. Proven failing against
...
def test_duplicate_sha_conflicts_after_the_first_was_rejected_by_the_scanner(tmp_path):
    ...
    FAILS IF the resubmit of an already REJECTED_AV package is accepted. Proven failing:
...
def test_eicar_upload_rejected_av_and_zip_deleted(tmp_path):
    # Proven failing: with the FOUND branch not deleting the zip, the file remained in
...
def test_clamd_down_holds_at_received_then_advances(tmp_path):
    ...
    # Proven failing: an early version advanced on ScanError (treating it as clean) -
...
def test_capacity_inflight_cap(tmp_path):
    # Proven failing: without the inflight gate the 2nd upload got 201 at max_inflight=1.
    async def _body():
