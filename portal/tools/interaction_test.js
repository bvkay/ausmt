...
(async () => {
  ...
  // T. C22 CITATION HONESTY (chief-architect ruling 2026-07-07; hostile-review finding 2026-07-06,
  // maintainer/HostileAudit-CommunityAdoption.md — state.js publisher placeholder). A NO-DOI survey's
  ...
  //
  // NOTE (Invariant 10): section T asserts the ASSEMBLY HELPERS (apa/bibtex/ris/citeLine) directly —
  ...
  ok(mBeta.doi === undefined && mBeta.cite === undefined,
    "T: fixture drift — Beta Survey must stay a no-cite/no-DOI survey for the no-DOI leg");
  const bibNo = A.bibtex("beta_survey", mBeta.cite || A.AUSMT_SELF, mBeta.doi);
  const risNo = A.ris(mBeta.cite || A.AUSMT_SELF, mBeta.doi);
  ok(bibNo.indexOf(PLACEHOLDER) < 0, "T: a no-DOI survey's .bib carries the placeholder string ('" + PLACEHOLDER + "'):\n" + bibNo);
  ok(risNo.indexOf(PLACEHOLDER) < 0, "T: a no-DOI survey's .ris carries the placeholder string ('" + PLACEHOLDER + "'):\n" + risNo);
  ok(!/\bdoi\s*=/.test(bibNo), "T: a no-DOI survey's .bib must have NO doi= line:\n" + bibNo);
  ok(!/^DO  - /m.test(risNo) && !/^UR  - /m.test(risNo), "T: a no-DOI survey's .ris must have NO DO/UR lines:\n" + risNo);
  ...
  ok(bibSelf.indexOf(PLACEHOLDER) < 0, "T: the catalogue self-citation .bib carries the placeholder:\n" + bibSelf);
  ok(risSelf.indexOf(PLACEHOLDER) < 0, "T: the catalogue self-citation .ris carries the placeholder:\n" + risSelf);
  ok(!/\bdoi\s*=/.test(bibSelf) && !/^DO  - /m.test(risSelf),
    "T: the catalogue self-citation must fabricate no DOI field");
  ...
  ok(mAlpha.doi === "10.99999/alpha-tf-doi",
    "T: fixture drift — Alpha Survey must carry the with-DOI fixture doi, got " + JSON.stringify(mAlpha.doi));
  const bibW = A.bibtex("alpha_survey", mAlpha.cite || A.AUSMT_SELF, mAlpha.doi);
  const risW = A.ris(mAlpha.cite || A.AUSMT_SELF, mAlpha.doi);
  ok(bibW.indexOf("doi       = {10.99999/alpha-tf-doi},") >= 0, "T: the with-DOI .bib lost its real doi= line:\n" + bibW);
  ok(risW.indexOf("DO  - 10.99999/alpha-tf-doi") >= 0 && risW.indexOf("UR  - https://doi.org/10.99999/alpha-tf-doi") >= 0,
    "T: the with-DOI .ris lost its real DO/UR lines:\n" + risW);
  ...
  ok(A.TS_COLLECTION.doi === "10.25914/mtjg-jp22", "T: TS_COLLECTION.doi drifted from 10.25914/mtjg-jp22");
  ...
  ok(lineNo.indexOf("[no DOI assigned]") >= 0,
    "T: the no-DOI CITATIONS.txt line must say [no DOI assigned], got: " + lineNo);
  ok(lineNo.indexOf(PLACEHOLDER) < 0, "T: the no-DOI CITATIONS.txt line still carries the placeholder: " + lineNo);
