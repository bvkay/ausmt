...
def test_a_capital_a_cut_left_behind_is_caught(tmp_path):
    ...
    The name half reads three shapes: a name the same file also spells in lower case, a bare FILE
    NAME (with or without a directory in front of it), and a bare module.function. The last two
    need no second spelling, because neither shape is a word English writes."""
    cases = {
        ...
        "a name wearing a capital it does not have (Alert.sh)":
            '"""Alert.sh already reads the drill record and publishes it."""\n',
        "a name wearing a capital it does not have (Scripts/verify.py)":
            '"""Scripts/verify.py --data-dir mode, over an existing build output."""\n',
        "a name wearing a capital it does not have (Edit._yaml)":
            "# Edit._yaml keeps the round-trip settings this suite pins.\nX = 1\n",
...
def test_a_library_class_keeps_the_capital_that_is_its_own(tmp_path):
    """The exemption the name rule needs, and the boundary of it. mt_metadata spells its classes
    Run and Channel, so `Run.id` at the head of a comment is the library's own spelling and not a
    scar; `Station.json` shares its stem with one of those classes and is still a file name, so the
    file-name shape is read first. engine/tests/test_presence_rule.py proves each exempted head is
    a class in the installed library."""
    ok = tmp_path / "fine.py"
    ok.write_text('"""Run.id where the source declares none: the station name plus a letter."""\n'
                  'NOTE = "run.id: a library default"\n', encoding="utf-8")
    assert not shape_offences([ok]), shape_offences([ok])
    ok.write_text("# Channel.contact_resistance defaults to 0.0 ohms on both bounds.\nX = 1\n",
                  encoding="utf-8")
    assert not shape_offences([ok]), shape_offences([ok])
    still = tmp_path / "still.py"
    still.write_text('"""Station.json is the served record."""\nS = "station.json"\n',
                     encoding="utf-8")
    hits = shape_offences([still])
    assert hits and "a name wearing a capital it does not have" in hits[0], hits
...
def test_the_subject_cut_rule_is_a_sampler_and_says_so(tmp_path):
    """One list of verbs serves both ends of the sentence, and it is a sampler: the shape a grammar
    would read fires on whole origin/main prose, because a plural noun stands in the same slot a cut
    leaves a verb in. What the sampler must catch is every verb it names, at either end; what it
    must not catch is the plural noun, the ellipsis and the abbreviation's dot."""
    for body in ('"""that is the enumeration defends against, so probe it."""\n',
                 '"""this is the corroboration accepts alongside the >INFO line."""\n',
                 '"""the file is the check documents as structurally blind."""\n',
                 '"""which is the divergence exists to remove."""\n'):
        f = tmp_path / "cut.py"
        f.write_text(body, encoding="utf-8")
        hits = shape_offences([f])
        assert hits and "subject was cut from the middle" in hits[0], (body, hits)

    head = tmp_path / "head.py"
    head.write_text('"""The tree is served as stored. refuses a station in an unresolvable frame."""\n',
                    encoding="utf-8")
    hits = shape_offences([head])
    assert hits and "opening on a verb with no subject" in hits[0], hits

    whole = tmp_path / "whole.py"
    whole.write_text(
        '"""An untracked survey dir is the incident class, the station list is the EDI files\n'
        '    themselves, and the fixture is the real corpus verbatim. The wording carries\n'
        '    "routes you straight to the archive that does" ... routes the reader onward, and\n'
        '    a wrong-signed round-trip i.e. proves that pin can fail."""\n', encoding="utf-8")
    assert not shape_offences([whole]), shape_offences([whole])


def test_two_sentence_marks_are_read_as_a_class(tmp_path):
    """Naming the pairs one at a time missed the next one by a single character three rounds
    running, so the rule reads the CLASS. What it may not read is the punctuation of a token the
    prose is naming, an operator a comment quotes, or an abbreviation's dot: each of those is
    written on purpose in this tree, and the last of them is a declared hole."""
    for pair, body in (
            (",;", "# Two overlapping ticks must not both build,; the second is a no-op.\na = 1\n"),
            (".,", "# STAGE-3b FIX ROUND (C)., each red-then-green.\na = 1\n"),
            (".:", "# emitted in both station.json and dimensionality.json.: station.json stopped.\na = 1\n"),
            (",:", "# a production regression,: the gate refuses it.\na = 1\n"),
            (";.", "# the runner is the only parser;. the gateway never is.\na = 1\n"),
            ("!,", "# it must NEVER be served!, and the gate says so.\na = 1\n")):
        f = tmp_path / "broken.py"
        f.write_text(body, encoding="utf-8")
        hits = shape_offences([f])
        assert hits and ("two sentence marks with nothing between them (%s)" % pair) in hits[0], \
            (pair, hits)

    whole = tmp_path / "whole.py"
    whole.write_text(
        "# The vendored copy resolves through conftest.py::resolve_validator_dir, compose keeps a\n"
        "# ${VAR:?} guard on these two, and the operations are {slug, op, block?:{id, title}}.\n"
        "# The accept list was '.edi,.h5,.mth5'; the schemes it flags are javascript:, data:, and a\n"
        "# bare word. The state reads as a glyph (tick/half/cross/?, not colour alone). Cite it\n"
        "# as Kay, B.; Heinson, G., e.g.: one entry per author, numbered 1., 2., 3.\n"
        "a = 1\n", encoding="utf-8")
    assert not shape_offences([whole]), shape_offences([whole])
...
def js_drivers():
    """Every node driver the portal runs: portal/tools/*.js and the *.test.js suites beside the
    Python guards. These are the only trees in this repository whose tests are JavaScript."""
    return [p for p in generators() + guard_tests() if p.suffix.lower() == ".js"]


def test_node_driver_messages_state_what_broke_without_the_audit_trail():
    """The node drivers' assertion messages, held to the same rule as the Python ones and for the
    same reason: an operator reads them on a red run. No other guard reaches them - the JS
    behaviour comparison holds a driver byte-identical apart from its comments, which freezes a
    message rather than reading it, and the Python message surface cannot parse a .js file."""
    files = js_drivers()
    assert files, "the driver class resolved to no files, so this would pass over nothing"
    read = sum(len(js_message_strings(source_text(p))) for p in files)
    assert read > 500, f"the driver message extractor read only {read} messages, so it is blind"
    hits = js_message_offences(files, root=ROOT)
    assert not hits, (
        f"{len(hits)} node-driver assertion message(s) carry the audit trail or a typographic "
        "dash:\n" + "\n".join(hits))


def test_a_tag_shape_inside_a_route_is_the_thing_the_route_addresses():
    """FAILS IF a station id at the end of a route reads as a work item, or if the exemption is so
    wide that a bare tag beside a slash escapes it. A route is told by its shape: it opens on / or
    #/ and carries at least two segments, and the token must be a PART of the run."""
    excused = "the driver opens #/station/au.alpha.A1 and reads the drawer"
    assert not work_item_tags(excused), work_item_tags(excused)
    for still in ("C42 is the coordinate-access work", "see /C42 for the shape",
                  "the C42/D3 seam"):
        assert work_item_tags(still), still


def test_the_driver_message_extractor_reads_the_message_and_not_the_data(tmp_path):
    """FAILS IF the extractor takes the condition, a nested call's arguments or a fixture literal
    for the message: a scanner that reads the data reports a station id as a work item, and one
    that reads nothing reports PASS over whatever is written."""
    driver = tmp_path / "driver.js"
    driver.write_text(
        'ok(rows.length === 2, "the two rows must render, got " + JSON.stringify(["A1", "A2"]));\n'
        'die("the boot never finished");\n'
        'ok(seen("ST01"), `the ${kind} row must carry ST01`);\n', encoding="utf-8")
    said = [text for _line, text in js_message_strings(driver.read_text(encoding="utf-8"))]
    assert said == ["the two rows must render, got ",
                    "the boot never finished",
                    "the ${kind} row must carry ST01"], said
