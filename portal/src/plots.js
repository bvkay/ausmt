"use strict";
// Pure SVG transfer-function plotters (no data/DOM dependency): ρ, φ, phase tensor, induction arrows. Each
// plotter takes a TF entry `t` (one per station, from tf.json). See docs: portal internals, plots.js.
...
// Open an <svg> whose viewBox carries the design coordinates and whose width/height carry that SAME design
// size (the viewBox is purely additive, so every pinned coordinate signature still matches). See docs:
...
// Vertical error bars. For each period draw a whisker between y(lo(v,e)) and y(hi(v,e)), only where BOTH
// the value and its error are present. See docs: portal internals, plots.js.
...
function phaseSvg(t){const per=t[T.periods];if(!per.length)return"";if(!t[T.phs_xy].some(v=>v!=null)&&!t[T.phs_yx_adj].some(v=>v!=null))return"";
  ...
  // Phase error bars in DEGREES (symmetric ± the propagated error). The yx error rides its +180°-adjusted
  // value (the error is orientation-independent). See docs: portal internals, plots.js.
...
// Plot kind registry. Each kind has a heading, an always-visible subline (the convention / axis-key text
// that must survive VISIBLY for the induction arrows, never hover-only), and an svg builder that takes only the TF
// row. See docs: portal internals, plots.js.
const PLOT_META={
...
// ONE expand control for the WHOLE response section. An expand button per plot block would give the
// drawer four controls for one action, because all four open the SAME full-station modal. See docs:
// portal internals, plots.js.
...
// are the collapsible body. Empty svg -> "". See docs: portal internals, plots.js.
...
// The expand affordance opens ONE full-station RESPONSE modal, not a single-plot popup. See docs: portal
// internals, plots.js.
const MODAL_PANELS=["rho","phase","pt","arrow"];   // stacked in this order (arrow absent for a non-tipper station)
// One modal panel: title + optional convention subline + svg + axis unit. An empty svg (arrowSvg for a
// non-tipper station, or any uncollected panel) yields "" so the panel is ABSENT, no empty box, mirroring
// plotBlock's guard. See docs: portal internals, plots.js.
