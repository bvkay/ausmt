...
function arrowPlot(t){const per=t[T.periods];if(!per.length)return"";
  ...
  const legend=arrow(refx,refTail+ref*scale,0,ref,scale,"#8FA3B0","1.1")+
    `<text x="${(refx+5).toFixed(1)}" y="${(refTail-2).toFixed(1)}" fill="#8FA3B0" font-size="8" font-family="monospace">|T|=0.5</text>`;
  return `<div class="plot"><div class="ptitle">Induction arrows - Parkinson convention (real arrows point toward conductors); imaginary unreversed.</div>`+
   ...
   decades(per).map(d=>`<text x="${x(d)}" y="${H-4}" fill="#8FA3B0" font-size="8.5" text-anchor="middle" font-family="monospace">${supTen(Math.round(Math.log10(d)))}</text>`).join("")+
   legend+g+
   `<rect x="${W-70}" y="2" width="8" height="8" fill="${REAL}"/><text x="${W-58}" y="9" fill="#8FA3B0" font-size="8">real</text>`+
   `<rect x="${W-38}" y="2" width="8" height="8" fill="${IMAG}"/><text x="${W-26}" y="9" fill="#8FA3B0" font-size="8">imag</text>`+
   `</svg></div>`;}
