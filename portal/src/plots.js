...
// UX6 Wave C: the SVG builders are viewBox-responsive (svgOpen threads a display scale `_k` onto the
// width/height while the viewBox holds the design coordinates), so the same plotter re-renders at ~2.5×
// in the expand modal without recomputing any geometry — the pinned <line/rect/path> signatures are
// scale-invariant. Series markers are shape-differentiated (xy = circle, yx = square) so the copper/teal
// pair is not colour-only. Curve COLOURS remain frozen (C20).
...
// UX6 Wave C: open an <svg> whose viewBox carries the design coordinates and whose width/height carry the
// DISPLAY size (design × _k). At _k=1 the width/height bytes are unchanged from the pre-C wave output
// (viewBox is purely additive), so every pinned coordinate signature still matches; at _k=2.5 the modal
// gets a crisp 2.5× render of the identical geometry.
function svgOpen(w,h,_k){_k=_k||1;return `<svg width="${Math.round(w*_k)}" height="${Math.round(h*_k)}" viewBox="0 0 ${w} ${h}" role="img">`;}
...
function rhoSvg(t,_k){const per=t[T.periods];if(!per.length)return"";const vals=[...t[T.rho_xy],...t[T.rho_yx]].filter(v=>v!=null&&v>0);if(!vals.length)return"";
  ...
  return svgOpen(W,H,_k)+frame(H,x,per,yl)+
   ...
   `<text x="${W-10}" y="14" fill="#E0782F" font-size="11" text-anchor="end" font-family="monospace">○ xy</text><text x="${W-10}" y="25" fill="#2E8FA3" font-size="11" text-anchor="end" font-family="monospace">□ yx</text></svg>`;}
function phaseSvg(t,_k){const per=t[T.periods];if(!per.length)return"";if(!t[T.phs_xy].some(v=>v!=null)&&!t[T.phs_yx_adj].some(v=>v!=null))return"";
  ...
  return svgOpen(W,H,_k)+frame(H,x,per,[[y(0),"0"],[y(45),"45"],[y(90),"90"]])+
...
function arrowSvg(t,_k){const per=t[T.periods];if(!per.length)return"";
...
function ptSvg(t,_k){const per=t[T.periods];if(!per.length||!t[T.pt_min].some(v=>v!=null))return"";const H=84,x=xScale(per),cy=37,maxR=23;let ell="";
...
// accepts a display scale. plotBlock renders the expanded in-drawer form (rho + phase); plotCollapsible
...
function expandBtn(kind){return `<button class="plotexp" type="button" data-act="expand" data-plot="${kind}" aria-label="Expand plot" title="Expand plot">⤢</button>`;}
const _paxis=`<div class="paxis">period (s) · log scale</div>`;   // x-axis unit, shared by all four plots
// Expanded block (rho + phase): title row (+ expand affordance) → optional subline → svg → axis unit.
function plotBlock(kind,t){const m=PLOT_META[kind];if(!m)return"";const svg=m.svg(t,1);if(!svg)return"";
  return `<div class="plot" data-plot="${kind}"><div class="ptitle">${m.title}${expandBtn(kind)}</div>`+
    (m.sub?`<div class="psubline">${m.sub}</div>`:"")+svg+_paxis+`</div>`;}
// Collapsed <details> block (phase tensor + induction arrows): the heading + subline sit in the summary
// (always visible); the svg + axis unit + expand affordance are the collapsible body. Empty svg -> "".
function plotCollapsible(kind,t,open){const m=PLOT_META[kind];if(!m)return"";const svg=m.svg(t,1);if(!svg)return"";
