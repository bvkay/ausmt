...
| `compose.yaml` | VPS | The one-service Caddy stack (host networking so it dials the box over the tailnet); mounts `Caddyfile.rendered` and `ts-routes.map.rendered`, both written in place by the installer. |
