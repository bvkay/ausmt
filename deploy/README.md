...
| **API requests** | Fetches of the four documented machine-readable entry points the portal's own JavaScript never fetches: `/data/products/manifest.json`, `/data/mtcat.json`, `/data/mtcat.schema.json` (the `$id` every validator resolves) and `/data/stations.geojson` (the vector layer a GIS opens from the URL). A **path class**. It is an **upper bound**: the mtcat link is in every page footer, so a human click lands here too. `/data/catalogue.json` stays the visit proxy and `/data/manifest.json` (the SPA's own boot fetch) is deliberately excluded. |
