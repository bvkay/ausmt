...
State-survey programs, institutional holdings and regional campaigns group the same way as
further surveys are published.
...
- EDI files (the submission and exchange format)
- EMTF XML files (canonical products generated by the build)
- MTH5 transfer-function bundles (generated by the build; the shipped `portal.config.yaml`
  turns this on, and a deployment can turn it off)
- Survey metadata
- Station metadata
- Provenance records
- Citation records
