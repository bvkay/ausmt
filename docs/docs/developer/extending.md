...
Ordered recipes. Each lists the files to touch in order and how to verify. Read
[Developer architecture](architecture.md) and [Portal data files](data-files.md) first.
...
1. **Prepare the package.** Either `cp -r ausmt-surveys/_example/example-survey
   ...
   the DMS HEAD/INFO conflict and writes `coordinate_resolution`), then upload to the gateway or
   download the package zip and unzip it under `surveys/<your-slug>/`. The `slug` must equal the folder
   name.
2. **Validate.**
   ...
   Fix any `FAIL`; WARNINGs do not block.
3. **Build the portal data.**
   ...
   mt_metadata is a required dependency on Python 3.12 (`environments/requirements-mtmetadata-lock.txt`);
   the build fails loudly without it and refuses to emit empty products unless `--allow-empty`.
4. **Review and publish.** Gateway submissions land in the curator queue and approval publishes the
   package as a git commit. For the manual path, open a pull request adding `surveys/<your-slug>/`; CI
   runs the validator and a curator reviews against the [Curator checklist](curator-checklist.md).
...
Regenerates a large demo from loose EDI folders. This path uses `state_of()` to split AusLAMP into
per-state surveys; survey-package mode does not, and raw mode is excluded from the incremental cache.

## 1. Add support for a new EDI dialect / processing code

mt_metadata already parses most dialects, including Phoenix EMpower cross-power SPECTRA-section EDIs.
Act only when it mis-reads a file or omits metadata AusMT relies on.

1. If mt_metadata reads the impedance but the canonical EMTF XML round-trip fails on metadata, condition
   it in `engine/ausmt_science/ingest/normalize.py` (`condition_tf`: the sanitisers for Site/Survey ids,
   `geographic_name`, citation, `rotation_angle`). If it cannot read the impedance at all, pin a newer
   mt_metadata or report upstream.
2. Header fields mt_metadata leaves empty live in `engine/extract/_edi_catalog.py`: `info_coords` and
   the HEAD/REF/INFO precedence in `coords_of` (coordinates + QC), `proc_info` (software, algorithm,
   remote-ref scrape), `parse_dataid` / `proc_note` (Phoenix `P=…R=…` DATAID to real station + remote
   site, plus the INFO note). Extend these for a new header convention.
...
2. Define the product's JSON in [Portal data files](data-files.md#the-key-based-documents) and emit it
   under `products/<survey>/<station>/<product>.json`, following how `build_portal` writes
   `station.json` / `dimensionality.json`.
3. Surface it in the portal: add a tile in `portal/src/drawer.js` `relatedProducts()`.
4. Verify: a unit test (synthetic input to expected output); move the product from planned to
   implemented in `science-products.md`.
...
1. `contract/columns.json`: append the column name (never reorder), then run
   ...
   `portal/src/contract.js`. Do not hand-edit either generated file.
...
1. `ausmt-surveys/_template/survey.yaml` and `_example/example-survey/survey.yaml`: add the field with
   a comment.
2. `docs/docs/reference/survey-yaml.md`: document it.
3. `ausmt-surveys/_validation/validate_survey.py`: add a rule (PASS/WARNING/FAIL) if it should be
   checked. The validator tolerates both schema generations; keep that.
...
Two settled conventions. A closed vocabulary fails closed: an out-of-vocabulary access level,
contributor role, identifier type or data level is a hard FAIL, never a warning or a silent coercion,
because these fields make claims about somebody else's data or rights. Retiring a field means adding a
migration, not deleting a reader: drop it from the editor UI, leave the engine reading it, add a
deprecation WARNING that fires only on a real value, and ship a script under `ausmt-surveys/_tools/`.
...
The portal can fetch data from a remote base by setting `deployment.data_base_url` in
...
and `_ediparse.py`, recorded in `build_provenance.json` by reading those constants. Change the constant,
never a re-typed literal. A threshold change alters scientific interpretation and requires a golden-test
diff and a scientific justification in the pull request; see the interpretation-sensitive list in
[Portal data files](data-files.md#interpretation-sensitive-operations).
