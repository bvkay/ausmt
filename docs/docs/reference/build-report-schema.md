...
`stations_dropped`, `warnings`, `conditioning`, `cache` and `duration_seconds`. Three more,
`xml_failures`, `ingest_sources` and `frame`, are optional.
...
### 2.9 surveys.<slug>.duration_seconds
